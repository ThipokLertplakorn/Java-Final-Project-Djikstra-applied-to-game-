...
public class Paths {
    ...
    public static List<Node> shortestPath(Node start, Node end) {
         ...
         */
    	List<Node> endNeighbors = new ArrayList<Node>();
    	for (Edge ed : end.getExits()){
    		if(ed.getFirstExit() != end)
				endNeighbors.add(ed.getFirstExit());
			else
				endNeighbors.add(ed.getSecondExit());
    	}
    	int trigger = endNeighbors.size();
    	
    	
    	System.out.println(endNeighbors);
	
    	LinkedList<Node> shortestPath = new LinkedList<Node>();
    	HashMap<Node,SFdata> map = new HashMap<Node,SFdata>();
    	Heap<Node> F = new Heap<Node>();
    	map.put(start, new SFdata(0,start));
    	map.get(start).backPointer = null;
    	
    	F.add(start, 0);
    	Node f = null;
    	while (F.size() != 0){
    		trigger = endNeighbors.size();
    		for (Node neighbor:endNeighbors){
    		...
    		for (Edge e:f.getExits()){
    			...
    			if (map.containsKey(w) == false){
    				map.put(w, new SFdata(e.length + map.get(f).distance,f));
    				F.add(w, map.get(w).distance);
    				...
    				if(map.get(f).distance + e.length < map.get(w).distance){
    					map.put(w, new SFdata(e.length + map.get(f).distance,f));
    					F.changePriority(w, map.get(w).distance);
    					//System.out.println(w);
    				}
    	...
    	while (map.get(end).backPointer != null){
    		shortestPath.addFirst(map.get(end).backPointer);
    		end = map.get(end).backPointer;
    		//System.out.print("working");
